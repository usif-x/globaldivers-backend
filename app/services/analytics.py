...
class AnalyticsServices:
    ...
    def get_dashboard_summary(self, month: int = None, year: int = None):
        ...
        # Revenue
        revenue_query = self.db.query(func.sum(Invoice.amount)).filter(
            Invoice.status == "PAID"
        )
        revenue_val = (
            apply_date_filter(revenue_query, Invoice.updated_at).scalar() or 0.0
        )

        # Sales Count
        sales_count_query = self.db.query(Invoice).filter(Invoice.status == "PAID")
        sales_count_val = apply_date_filter(
            sales_count_query, Invoice.updated_at
        ).count()
        ...
        total_discount_query = self.db.query(func.sum(Invoice.discount_amount)).filter(
            Invoice.status == "PAID"
        )
        total_discount_given = (
            apply_general_filter(total_discount_query).scalar() or 0.0
        )
        ...
        chart_end_date = (
            end_date if is_filtered_view else today
        )  # or unlimited? typically up to now.
        ...
        top_users_query = apply_general_filter(
            top_users_query
        )  # Details from Invoice.updated_at usually
