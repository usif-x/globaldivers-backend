...
from app.core.security import create_user_access_token, create_admin_access_token
  ...
  def user_login(self, user: UserLogin):
    ...
    if verify_password(user.password, logged_user.password):
        ...
        return {
            ...
            "token": create_user_access_token(logged_user.id, login_date.isoformat()),
  ...
  def admin_login(self, admin: AdminLogin):
    ...
    if verify_password(admin.password, logged_admin.password):
      data = {"success": True, 
              ...
              "token_type": "Bearer", 
              "admin": AdminResponse.model_validate(logged_admin,from_attributes=True)}
      return data
    else:
      raise HTTPException(400, detail="Invalid cerdentials")