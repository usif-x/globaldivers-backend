...
testimonial_routes = APIRouter(
  prefix="/testimonials",
  tags=["Testimonial Endpoints"]
)



@testimonial_routes.post("/create")
async def create_testimonial(testimonial: CreateTestimonial, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    return TestimonialServices(db).create_testimonial(testimonial, user.id)
...
async def get_testimonial_by_id(id: int = Path(..., ge=1), db: Session = Depends(get_db)):
