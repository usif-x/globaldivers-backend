...
@package_routes.post("/", response_model=PackageResponse, dependencies=[Depends(get_current_admin)])
...
async def get_all_packages(db: Session = Depends(get_db)):
    return await run_in_threadpool(PackageServices(db).get_all_packages)
...
async def get_package_by_id(id: int = Path(..., ge=1), db: Session = Depends(get_db)):
    return await run_in_threadpool(PackageServices(db).get_package_by_id, id)


@package_routes.delete("/all", dependencies=[Depends(get_current_admin)])
async def delete_all_packages(db: Session = Depends(get_db)):
    return await run_in_threadpool(PackageServices(db).delete_all_packages)


@package_routes.delete("/{id}", dependencies=[Depends(get_current_admin)])
async def delete_package(id: int, db: Session = Depends(get_db)):
    return await run_in_threadpool(PackageServices(db).delete_package, id)
...
async def get_trip_by_package_id(
    ...
    return await run_in_threadpool(PackageServices(db).get_trip_by_package_id, id)
