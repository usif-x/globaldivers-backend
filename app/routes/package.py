...
async def update_package(package: UpdatePackage, id: int, db: Session = Depends(get_db)):
    return PackageServices(db).update_package(package, id)



@package_routes.get("/{id}/trips", response_model=list[TripResponse])
async def get_trip_by_package_id(id: int = Path(..., ge=1), db: Session = Depends(get_db)):
