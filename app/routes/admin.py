...
@admin_routes.get("/get-all-users", response_model=list[UserResponse], dependencies=[Depends(get_current_admin)])
...
@admin_routes.get("/get-all-admins", response_model=list[AdminResponse] , dependencies=[Depends(get_current_super_admin)])
async def get_all_admins(db: Session = Depends(get_db)):
    return AdminServices(db).get_all_admins()


@admin_routes.put("/update")
async def update_admin(admin: AdminUpdate, db: Session = Depends(get_db), current_admin: Admin = Depends(get_current_admin)):
...
async def update_admin_password(id: int, admin: AdminUpdatePassword, db: Session = Depends(get_db)):
    return AdminServices(db).update_admin_password(admin, id)

@admin_routes.put("/update/password")
async def update_admin_password(admin: AdminUpdatePassword, db: Session = Depends(get_db), current_admin: Admin = Depends(get_current_admin)):
    return AdminServices(db).update_admin_password(admin, current_admin.id)

@admin_routes.put("/update-admin/{id}/password", dependencies=[Depends(get_current_super_admin)])
async def update_admin_password(id: int, admin: AdminUpdatePassword, db: Session = Depends(get_db)):
    return AdminServices(db).update_admin_password(admin, id)

@admin_routes.delete("/delete-admin/{id}", dependencies=[Depends(get_current_super_admin)])
...
async def edit_user_information(id: int, user: UserUpdate, db: Session = Depends(get_db)):
    return AdminServices(db).edit_user_information(id, user)




@admin_routes.get("/get-user-testminals/{id}", dependencies=[Depends(get_current_admin)])
